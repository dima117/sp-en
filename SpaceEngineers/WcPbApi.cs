...
namespace SpaceEngineers
{
    ...
    public class WcPbApi
    {
        ...
        public bool Activate(IMyTerminalBlock pBlock)
        {
            this.pBlock = pBlock;

            var dict = pBlock.GetProperty("WcPbAPI")?.As<IReadOnlyDictionary<string, Delegate>>().GetValue(pBlock);
        ...
        public void GetObstructions(ICollection<Sandbox.ModAPI.Ingame.MyDetectedEntityInfo> collection) =>
            _getObstructions?.Invoke(pBlock, collection);
        public MyDetectedEntityInfo? GetAiFocus(long shooter, int priority = 0) => _getAiFocus?.Invoke(shooter, priority);

        public bool SetAiFocus(long target, int priority = 0) =>
            _setAiFocus?.Invoke(pBlock, target, priority) ?? false;
        public bool ReleaseAiFocus(long playerId) =>
